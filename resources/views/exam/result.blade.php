...
<div class="box">
    ...
    <div class="content">
        ...
        <p>Kỳ Thi: {{ $exam->semester->name }}</p>
        <p>Môn Thi: {{ $exam->subject->name }}</p>
        <p>Điểm: {{ $point }} điểm</p>
        <p>Số câu đúng: {{ $numCorrect }}/{{ $totalQuestion }} câu</p>
        <p>Hoàn thành: {{ $point * 10 }}%</p>
        <p>Thời Gian Thi: {{ $exam->time }} phút</p>
        <p>
            @if($point <= 5)
                " Cần cố gắng hơn nữa!! "
            @elseif($point > 5 && $point < 8)
                " Cần cố gắng! "
            @elseif($point >= 8)
                " Làm tốt lắm !!! "
            @endif
...
<a href="../lichsuthi/{{ $exam->id }}">
    <div class="login"> Lịch Sử Bài Làm</div>
</a>
